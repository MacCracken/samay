...
**Samay** (Hindi: समय — time) — Task scheduler for AGNOS (cron, resource-aware scheduling)
...
daimon (task scheduling), kavach (sandboxed execution of scheduled tasks). Samay provides cron-like scheduling with resource-awareness via ai-hwaccel for GPU/accelerator-conscious task placement.
...
- Scheduling must be deterministic — same schedule + same time = same decisions
- Resource-aware placement via ai-hwaccel — never schedule GPU tasks without checking availability
- Missed schedules must be handled explicitly — configurable catch-up vs skip policy
- Cron expressions must be validated at parse time, not at execution time
...
- Do not silently skip missed schedules — always log and follow configured policy
- Do not schedule tasks without resource availability checks
